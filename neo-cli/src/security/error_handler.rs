...
impl RetryHandler {
    ...
    pub async fn retry<F, Fut, T>(&self, operation: F) -> Result<T, CliError>
    where
        F: Fn() -> Fut,
        Fut: Future<Output = Result<T, CliError>>,
    {
        ...
        Err(last_error.unwrap_or_else(|| {
            CliError::Network("Max retry attempts exceeded".to_string())
        }))
    }
...
pub struct ErrorHandler {
    ...
    circuit_breakers: std::sync::Arc<std::sync::Mutex<std::collections::HashMap<String, CircuitBreaker>>>,
}
...
impl ErrorHandler {
    pub fn new() -> Self {
        Self {
            ...
            circuit_breakers: std::sync::Arc::new(std::sync::Mutex::new(std::collections::HashMap::new())),
        }
    }
