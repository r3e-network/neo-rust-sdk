...
use crate::{
    ...
    neo_types::{ScriptHash, ContractParameter, StackItem},
    neo_builder::{TransactionBuilder, ScriptBuilder},
...
impl Neo {
    ...
    pub async fn get_balance(&self, address: &str) -> Result<Balance, NeoError> {
        ...
        let neo_balance = self.client.invoke_function(
            &neo_hash,
            "balanceOf".to_string(),
        ...
        let gas_balance = self.client.invoke_function(
            &gas_hash,
            "balanceOf".to_string(),
            vec![ContractParameter::h160(&script_hash)],
            None
