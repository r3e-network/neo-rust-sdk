...
async fn check_block_height(client: &RpcClient<HttpProvider>) -> Result<String, Box<dyn std::error::Error>> {
    let height = client.get_block_count().await?;
    Ok(format!("{} blocks", height))
}

async fn check_mempool(client: &RpcClient<HttpProvider>) -> Result<String, Box<dyn std::error::Error>> {
    let mempool = client.get_raw_mempool().await?;
    Ok(format!("{} pending transactions", mempool.len()))
}

async fn check_connections(client: &RpcClient<HttpProvider>) -> Result<String, Box<dyn std::error::Error>> {
    let connection_count = client.get_connection_count().await?;
    Ok(format!("{} peers", connection_count))
}

async fn check_sync_status(client: &RpcClient<HttpProvider>) -> Result<String, Box<dyn std::error::Error>> {
...
async fn example_performance_metrics() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let operations = vec![
        ("get_block_count", measure_operation_latency(&client, |c| {
            Box::pin(async move { c.get_block_count().await.map(|_| ()) })
        }).await),
        ("get_version", measure_operation_latency(&client, |c| {
            Box::pin(async move { c.get_version().await.map(|_| ()) })
        }).await),
        ("get_raw_mempool", measure_operation_latency(&client, |c| {
